...
- **Ui.elm**: `web/src/Ui.elm` centralizes class names for shared components as `Html.Attribute` helpers (`Ui.primaryButton`, `Ui.card`, …). Use these instead of raw class literals for anything reusable; page-specific one-off classes may stay inline. Add the CSS and the helper together
